        ...
        <v-layout row wrap>
          <v-card color="dark" elevation-24 style="padding: 20px; border: 1px; border-radius: 30px;">
            <v-card-text>
              ...
              <br>
              <v-form>
                <v-text-field clearable v-model="email" label="이메일" prepend-icon="face" type="email" color="#b3e3c3"
                  @keyup.enter="enter()">
                </v-text-field>
                <v-text-field clearable v-model="password" label="비밀번호" prepend-icon="lock" type="password"
                  color="#b3e3c3" counter="20" @keyup.enter="enter()">
                </v-text-field>
              </v-form>
              <br>
              <div id="login_btn">
                <v-btn depressed @click="login()" width="378" color="#b3e3c3" large style="font-weight:bold">로그인</v-btn>
              </div>
            </v-card-text>
            <hr>
            <ul>
