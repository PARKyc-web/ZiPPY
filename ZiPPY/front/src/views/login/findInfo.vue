            ...
            <v-card-text>
              ...
              <div id="typeBtn">
                <b-form-radio-group id="btn-radios-2" v-model="selected" :options="options"
                  button-variant="outline-success" size="sm" name="radio-btn-outline" buttons></b-form-radio-group>
              </div>

              <br>
              <v-form v-if="selected == 'radio1'">
                <v-text-field clearable label="사용자 이름(CEO 이름)" prepend-icon="person" type="text" color="#b3e3c3"
                  id="userName">
                </v-text-field>

                <v-text-field id="find_phone" clearable label="핸드폰 번호" prepend-icon="phone_iphone" type="number"
                  color="#b3e3c3">
                </v-text-field>
                <div id="login_btn">
                  <v-btn @click="sendValidationNum(1)" color="#b3e3c3" depressed large style="font-weight:bold">인증번호 받기
                  </v-btn>
                </div>
                <br>
                <v-text-field id="phoneValid" clearable label="인증번호" prepend-icon="lock" type="number" color="#b3e3c3">
                </v-text-field>
              </v-form>

              <v-form v-if="selected == 'radio2'">
                <v-text-field clearable id="email" label="이메일" prepend-icon="email" type="email" name="userEmail"
                  color="#b3e3c3">
                </v-text-field>
              </v-form>
              <br>
              ...
              <div id="login_btn" v-if="selected == 'radio2'">
                <v-btn type="button" @click="sendValidationNum(0)" color="#b3e3c3" depressed large
                  style="font-weight:bold">비밀번호 찾기</v-btn>
              </div>
            </v-card-text>
